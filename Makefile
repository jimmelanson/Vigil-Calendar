# This Makefile is for the Vigil::Calendar extension to perl.
#
...
#     ABSTRACT => q[Calendar module for Perl applications]
#     AUTHOR => [q[Jim Melanson <jmelanson1965@gmail.com>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     LICENSE => q[perl]
#     MIN_PERL_VERSION => q[5.010000]
#     NAME => q[Vigil::Calendar]
#     PREREQ_PM => { Time::Local=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Vigil/Calendar.pm]
...
MAKE = gmake
FULLEXT = Vigil\Calendar
BASEEXT = Calendar
PARENT_NAME = Vigil
DLBASE = $(BASEEXT)
VERSION_FROM = lib/Vigil/Calendar.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
INST_LIBDIR      = $(INST_LIB)\Vigil
INST_ARCHLIBDIR  = $(INST_ARCHLIB)\Vigil
...
TO_INST_PM = lib/Vigil/Calendar.pm
...
DISTNAME = Vigil-Calendar
DISTVNAME = Vigil-Calendar-2.1.2
...
Calendar.def: Makefile.PL
	$(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"Vigil::Calendar\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) "abstract: 'Calendar module for Perl applications'" >> META_new.yml
	$(NOECHO) $(ECHO) author: >> META_new.yml
	$(NOECHO) $(ECHO) "  - 'Jim Melanson <jmelanson1965@gmail.com>'" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "name: Vigil-Calendar" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "version: v2.1.2" >> META_new.yml
	$(NOECHO) $(ECHO) "x_serialization_backend: 'CPAN::Meta::YAML version 0.018'" >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) { > META_new.json
	$(NOECHO) $(ECHO) "   \"abstract\" : \"Calendar module for Perl applications\"," >> META_new.json
	$(NOECHO) $(ECHO) "   \"author\" : [" >> META_new.json
	$(NOECHO) $(ECHO) "      \"Jim Melanson ^<jmelanson1965^@gmail.com^>\"" >> META_new.json
	$(NOECHO) $(ECHO) "   ]," >> META_new.json
	...
	$(NOECHO) $(ECHO) "   }," >> META_new.json
	$(NOECHO) $(ECHO) "   \"name\" : \"Vigil-Calendar\"," >> META_new.json
	...
	$(NOECHO) $(ECHO) "   \"version\" : \"v2.1.2\"," >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) "<SOFTPKG NAME=\"Vigil-Calendar\" VERSION=\"2.1.2\">" > Vigil-Calendar.ppd
	$(NOECHO) $(ECHO) "    <ABSTRACT>Calendar module for Perl applications</ABSTRACT>" >> Vigil-Calendar.ppd
	$(NOECHO) $(ECHO) "    <AUTHOR>Jim Melanson &lt;jmelanson1965@gmail.com&gt;</AUTHOR>" >> Vigil-Calendar.ppd
	$(NOECHO) $(ECHO) "    <IMPLEMENTATION>" >> Vigil-Calendar.ppd
	$(NOECHO) $(ECHO) "        <PERLCORE VERSION=\"5,010000,0,0\" />" >> Vigil-Calendar.ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Time::Local\" />" >> Vigil-Calendar.ppd
	$(NOECHO) $(ECHO) "        <ARCHITECTURE NAME=\"MSWin32-x64-multi-thread-5.32\" />" >> Vigil-Calendar.ppd
	$(NOECHO) $(ECHO) "        <CODEBASE HREF=\"\" />" >> Vigil-Calendar.ppd
	$(NOECHO) $(ECHO) "    </IMPLEMENTATION>" >> Vigil-Calendar.ppd
	$(NOECHO) $(ECHO) ^</SOFTPKG^> >> Vigil-Calendar.ppd
...
pm_to_blib({@ARGV}, '$(INST_LIB)\auto', $$filter, '$(PERM_DIR)')" -- $(PM_FILTER) -- \
	  lib/Vigil/Calendar.pm blib\lib\Vigil\Calendar.pm 
